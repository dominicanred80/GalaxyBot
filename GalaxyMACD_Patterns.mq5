...
//+------------------------------------------------------------------+
//| CONFIRMED via direct visual review: the signal this bot trades on |
//| is the MAIN LINE crossing zero - the blue arrows below. That's    |
//| the only signal type this indicator shows now. The earlier        |
//| three-way comparison (signal line crosses zero, histogram crosses |
//| zero) has been removed entirely, not just hidden, since those      |
//| were only ever needed to help pin down which one was correct.     |
//|                                                                    |
//| Plots, in a separate lower window:                                 |
//|   - Main line   (electric blue) = EMA(fast) - EMA(slow)             |
//|   - Signal line (amber)         = SMA(main, signal period) - MT5's |
//|                                    real native convention           |
//|   - Histogram (4-color, main minus signal) - a "momentum breathing"|
//|     style: vivid when building, muted when fading, distinct colors |
//|     above vs below zero. Visual context only - not a trade signal. |
//|   - Bold arrows exactly where the main line crosses zero - bright  |
//|     aqua triangle up for a buy, royal blue triangle down for a     |
//|     sell. This is the ONLY signal marked here, and exactly the     |
//|     entry condition the EA actually trades on.                     |
//|                                                                    |
//| NEW: flag/triangle pattern detection (InpShowPatterns), drawn on   |
//| the MAIN price chart, not this subwindow - purely visual/          |
//| informational, no trading logic depends on it. Scans the most      |
//| recent InpPatternLookback bars (default 25) for swing highs/lows,  |
//| fits a simple line through each side, and classifies Ascending/    |
//| Descending/Symmetrical Triangle or Bull/Bear Flag (flag requires a |
//| genuine prior "pole" move, not just a drifting channel). HONEST    |
//| NOTE: at this short a window there's only room for 2-3 genuine     |
//| swing points per side, not the 4-5 a longer window would give -    |
//| this trades statistical confidence for catching fast, intraday     |
//| formations. Every threshold is a reasoned starting point, not      |
//| validated against real data - inherently fuzzier than the bot's    |
//| mechanical MACD-based logic.                                       |
//+------------------------------------------------------------------+
#property strict
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Galaxy MACD - Confirmed Signal");
...
void OnDeinit(const int reason)
{
   ...
   int win = (subWin >= 0) ? subWin : ChartWindowFind(0, "Galaxy MACD - Confirmed Signal");
   if(win >= 0)
   {
                ...
                const int &spread[])
{
   ...
   if(subWin < 0)
      subWin = ChartWindowFind(0, "Galaxy MACD - Confirmed Signal");
